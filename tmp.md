...
백앤드 개발을 위해 사용할 수 있는 언어와 프레임워크는 정말 많다. 파이썬의 장고이거나 자바의 스프링을 비롯한 다른 언어의 프레임워크도 쓸 수 있겠지만 개인적으로 자유도가 높은 마이크로 프레임워크가 좋았기 때문에 나는 파이썬과 플라스크의 조합을 선택했다.
플라스크는 마이크로 프레임워크다. 정말 필수적인 기능만을 제공하여 장고와 같은 풀스택 프레임워크와 달리 애플리케이션을 구성하는 방법이 자유로운데 이는 개발자에게 장점으로 다가올 수도 있겠지만 반대로 혼란을 만들 수도 있다. 하지만 플라스크를 잘 사용한 어플리케이션은 장고 같은 풀스택 프레임워크로 만든 어플리케이션 못지않게 강력하게 될 수 있다.
앞으로 소개할 것들은 나의 경험을 바탕으로 기초에서 벗어나 플라스크 어플리케이션을 강력하게 만드는 법에 대한 것과 응용하여 만든 간단한 예시들이다. 여기서 소개하는 것들은 각각으로도 글을 쓸 수 있을 만큼 큰 내용들이지만 허용하는 선에서 축약해 소개하고자 한다.
...
2. 블루프린트는 코드 모듈화 및 api 버전 관리를 위해 사용한다.
...
이 데코레이터들은 모두 뷰 펑션 실행 전에 사용되는 코드들이기 때문에 리스폰스 압축 데코레이터를 만들 수 있을지 의문이 들수 있지만 after_this_request 콜백을 사용하면 데코레이터에 뷰 펑션 실행 후에 사용되는 코드를 삽입할 수 있다. 

다음은 리스폰스를 gzip알고리즘으로 압축하게 하여 주는 뷰 데코레이터의 예시이다.
...
2. 뷰 데코레이터는 다음과 같은 동작을 위해 사용하자
    ...
    3. 캐싱
    4. 리스폰스 압축
3. 로직 후에 실행되어야 하는 코드는 데코레이터 안에서 after_this_request 콜백을 적용하자.
...
2. 400~403 번대의 에러는 바로 상태코드를 주는 것 대신 예외를 만들고 에러핸들러로 처리하자
3. 경우에 따라 다른 에러핸들링이 필요할 경우 블루프린트 별로 에러핸들러를 등록하자


## BETTER WAY 05: 컨텍스트를 잘 사용하자
...
## BETTER WAY 07: flasgger를 사용하여 API를 문서화하자
<!-- 프론트와의 협업의 기준 -->

## BETTER WAY 08: 자주 사용하는 코드를 플라스크 익스텐션으로 만들어 사용하자
<!-- WSGI랑 유니콘같은거ㅏ -->
