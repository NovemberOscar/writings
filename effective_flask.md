...
## Beginner 들에게
고등학생 백앤드 개발자로써 플라스크를 이용한 백앤드 개발을 하면서 많은 점들을 느꼈다.
플라스크 뿐만 아니라 다른 모든 분야에서도 가장 좋은 방법은 좋은 예제들을 많이 보는 것이다. 

고등학생으로 백앤드 개발자 되기 위해 고생 함 가장 좋은 것은 좋은 예제들을 많이 보는것, 그리고 항상 검색하는 습관을 들이고 새로운 것을 계속 해 보면서 프로젝트 하는것 내가 하는 일들은 익스텐션 개발 팀 / 아이디어를 개발하는것 엔트리 오졌음 트래픽 많은 환경이 있었으면 좋겠다 대회 많이 인프라에 신경 쓰자 / 
이것들 내가 공부하며 깨달음
그런데 사람들이 나처럼 많이 햇갈리는 것 같음
