...
프로젝트를 하는것에 막연한 두려움을 가지지 말고 간단한 아이디어라도 직접 만들어 보자. 또 프로젝트를 진행하면서 어느정도 플라스크에 익숙해 졌다면 REST API 대신 GraphQL을 시도해 보거나 MySQL 대신 MongoDB를 사용해 보고 Flask와 유사한 경험을 제공하는 비동기 프레임워크 Sanic으로의 마이그레이션 등 새로운 것들을 시도해 보자.  

지면상의 한계로 인해 더 많은 것들을 소개하지 못해 아쉽지만 마지막으로 플라스크 애플리케이션 개발에 도움이 될 수 있는 자료와 익스텐션들을 간단히 소개하고자 한다. 
